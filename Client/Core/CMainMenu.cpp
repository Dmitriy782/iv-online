...
void CMainMenu::SetVisible(bool bVisible)
{
	...
	SetConnectedState(pNetworkManager != NULL && pNetworkManager->IsConnected());
...
void CMainMenu::SetConnectedState(bool isConnected)
{
	m_pDisconnectButton->setVisible(isConnected);
	m_bDisconnectButtonVisible = isConnected;

	m_pServerBrowserButton->setVisible(!isConnected);
	m_pQuickConnectButton->setVisible(!isConnected);
}
