...
CDirectInput8Proxy::CDirectInput8Proxy(IDirectInput8 * dinput, DirectInputHandlerInterface * handler)
{
	m_dinput = dinput;
	m_Handler = handler;
}
...
HRESULT STDMETHODCALLTYPE CDirectInput8Proxy::CreateDevice(REFGUID rguid, LPDIRECTINPUTDEVICE8A * lplpDirectInputDevice, LPUNKNOWN pUknOuter)
{
	...
	if(SUCCEEDED(hr))
	{
		...
		*lplpDirectInputDevice = new CDirectInputDevice8Proxy(this, *lplpDirectInputDevice, type, m_Handler);
	}
	else
