...
int CHashNatives::md5(lua_State * pVM)
{
	const char * toHash;
	char result[33];
	MD5 md5bytes;
	...
	script_getstring(pVM, -1, &toHash);

	hasher.Calculate(toHash, strlen(toHash), md5bytes);
	hasher.ConvertToHex(md5bytes, result);

	script_pushlstring(pVM, result, 32);

	return 1;
}

int CHashNatives::md5File(lua_State * pVM)
{
	const char * szFileName;
	char result[33];
	MD5 md5bytes;
	CMD5Hasher hasher;
	script_getstring(pVM, -1, &szFileName);
