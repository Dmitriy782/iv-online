...
bool CClient::ProcessInputDeviceState(eDIDeviceType dt, char * keyBuffer, int size, HRESULT oldHResult)
{
	if(dt == DIDEVICE_TYPE_KEYBOARD)
	{
		if(keyBuffer[DIK_ESCAPE] != m_InputState[DIK_ESCAPE])
		{
			if((keyBuffer[DIK_ESCAPE] & 0x80) == 0x80)
			{
				if(CGame::GetState() == GAME_STATE_INGAME)
				{
					...
					{
						if(m_pPlayerManager->DoesExist(i))
							players++;
					}

					if(m_pNetworkManager->IsConnected())
						m_pMainMenu->SetNetworkStats(m_pNetworkManager->GetHostName(), players, m_pNetworkManager->GetMaxPlayers(), m_pLocalPlayer->GetName());
				}
				else if(CGame::GetState() == GAME_STATE_IVMP_PAUSE_MENU)
				{
					CGame::SetState(GAME_STATE_INGAME);
					CGame::SetInputState(true);
					m_pChatWindow->SetEnabled(true);

					if(m_pLocalPlayer && !m_pLocalPlayer->IsRadarVisible())
						CGame::SetRadarVisible(false);
					else
						CGame::SetRadarVisible(true);
				}
			}
			m_InputState[DIK_ESCAPE] = keyBuffer[DIK_ESCAPE];
		}
	...
	if(dt == DIDEVICE_TYPE_KEYBOARD)
	{
		...
		{
			m_InputState[DIK_RETURN] = !m_InputState[DIK_RETURN];
			keyBuffer[DIK_RETURN] = m_InputState[DIK_RETURN];
		}
	}
