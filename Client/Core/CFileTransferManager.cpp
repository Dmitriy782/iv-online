...
void CFileTransferManager::Add(CMetaFile & mf, CFileChecksum checksum)
{
	...
	CFileTransfer * pFileTransfer = new CFileTransfer(m_strHost, m_usPort, mf, checksum);
	...
	CLogFile::Printf("Added file %s to transfer list", mf.ToMetaAndFileString().Get());
...
bool CFileTransferManager::Remove(CMetaFile & mf)
{
...
// 6/16/2013, mryaznaov@lab322.com, Fixed: invalid iterator after removal
void CFileTransferManager::Process()
{
	...
	for(std::list<CFileTransfer *>::iterator i = m_transferList.begin(); i != m_transferList.end(); /*i++*/)
	{
		...
		if(pFileTransfer->IsComplete())
		{
			...
			{
				if(pFileTransfer->GetMetaFileA().m_Type != MetaFileType_ClientResource)
				{
					CLogFile::PrintDebugf("Adding client script: %s.", pFileTransfer->GetMetaFile().ToMetaAndFileString().Get());
					CClientScriptManager * pClientScriptManager = g_pClient->GetClientScriptManager();
					pClientScriptManager->AddScript(pFileTransfer->GetMetaFileA());

					if(g_pClient->GetLocalPlayer()->GetFirstSpawn())
						pClientScriptManager->Load(pFileTransfer->GetMetaFile());
				}
			}
			else
			{
				String strErrorMessage("Failed to download file %s (%s)", pFileTransfer->GetMetaFile().ToMetaAndFileString().Get(), pFileTransfer->GetError().Get());
...
void CFileTransferManager::TransferThread(CThread * pCreator)
{
	...
	if(!SharedUtility::Exists(strFolderName + "\\" + IVMP_RESOURCES_FOLDER))
		SharedUtility::CreateDirectoryA(strFolderName + "\\" + IVMP_RESOURCES_FOLDER);	
	...
	for(std::list<CFileTransfer *>::iterator i = pThis->m_transferList.begin(); i != pThis->m_transferList.end(); i++)
	{
		...
		// Download our file
		CLogFile::Printf("TransferThread: pFileTransfer(%s) Downloading...", pFileTransfer->GetMetaFile().ToMetaAndFileString().Get());
		if(pFileTransfer->Download())
			CLogFile::Printf("TransferThread: pFileTransfer(%s) Done", pFileTransfer->GetMetaFile().ToMetaAndFileString().Get());
		else
			CLogFile::Printf("TransferThread: pFileTransfer(%s) Failed: %s", pFileTransfer->GetMetaFile().ToMetaAndFileString().Get(), pFileTransfer->GetError().Get());
