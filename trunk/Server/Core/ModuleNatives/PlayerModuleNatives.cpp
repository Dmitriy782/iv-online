...
namespace Modules
{
	...
	bool CPlayerModuleNatives::TriggerEvent(EntityId playerid, const char * szEventName, const char * szFormat, ScriptArgumentsInterface* args, ...)
	{
		if(args != NULL)
		{
			// If we have arguments already, we add event name to the front of list and send them to client.
			CScriptArguments* arguments = reinterpret_cast<CScriptArguments*>(args);
			arguments->push_front((new CScriptArgument(String(szEventName))));
			CBitStream bsSend;
		...
		CBitStream bsSend;
		CScriptArguments* arguments = new CScriptArguments();
