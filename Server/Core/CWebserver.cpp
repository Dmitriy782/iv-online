...
bool CWebServer::FileCopy(CMetaFile & mf, CFileChecksum &fileChecksum)
{
	String src = mf.ToFullPath();

	// Do we not have an external Webserver configured?
	if(CVAR_GET_STRING("httpserver").IsEmpty())
	{
		String strWebServerFolder(SharedUtility::GetAbsolutePath("webserver"));
		String dst("%s\\"IVMP_RESOURCES_FOLDER"\\%s\\%s", strWebServerFolder.Get(), mf.m_Meta.Get(), mf.m_File.Get());
		...
		String dstResourcesFolder("%s\\"IVMP_RESOURCES_FOLDER"\\", strWebServerFolder.Get());
		if(!SharedUtility::Exists(dstResourcesFolder.Get()))
			SharedUtility::CreateDirectory(dstResourcesFolder.Get());

		// Create the web server meta folder if needed
		String dstMetaFolder("%s\\"IVMP_RESOURCES_FOLDER"\\%s", strWebServerFolder.Get(), mf.m_Meta.Get());
		if(!SharedUtility::Exists(dstMetaFolder.Get()))
			SharedUtility::CreateDirectory(dstMetaFolder.Get());

		if(!SharedUtility::CopyFile(src.Get(), dst.Get()))
			return false;
	}

	return fileChecksum.Calculate(src.Get());
}
