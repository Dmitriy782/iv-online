...
CScriptTimer::CScriptTimer(CScript* pScript, int pFunction, int uiInterval, int iRepeations, CScriptArguments* pArguments)
{
	m_bIsDead = false;
	m_pScript = pScript;
...
bool CScriptTimer::Pulse()
{
	...
	{
		// call the function
		m_pScript->Call(m_pFunction, m_pArguments, NULL, "Timer Pulse");
...
CScript* CScriptTimer::GetScript()
{
	return m_pScript;
}
