...
void CClientRPCHandler::NewFile(CBitStream * pBitStream, CPlayerSocket * pSenderSocket)
{
	...
	// Read the file type
	while(pBitStream->Read((char &)type))
	{
		// Read the file name
		pBitStream->Read(meta);
		pBitStream->Read(file);

		// Read the file checksum
		pBitStream->Read((char *)&fileChecksum, sizeof(CFileChecksum));

		// Add the file to the file transfer manager
		g_pClient->GetFileTransfer()->Add(CMetaFile(meta, file, type), fileChecksum);
	}
}
...
void CClientRPCHandler::DeleteFile(CBitStream * pBitStream, CPlayerSocket * pSenderSocket)
{
	...
	CMetaFile mf(meta, file, type);

	if((type == MetaFileType_ClientScript || type == MetaFileType_ServerScript)  && pClientScriptManager->Exists(mf))
	{
		pClientScriptManager->Unload(mf);
		pClientScriptManager->RemoveScript(mf);
	}
}
