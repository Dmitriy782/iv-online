	...
	if(m_iSocket != -1)
	{
		...
		while((iBytesRead = recvfrom(m_iSocket, szBuffer, sizeof(szBuffer), NULL, (sockaddr *)&addr, (socklen_t *)&iFromLen)) != -1)
		{
			...
			if(CVAR_GET_BOOL("frequentevents"))
			{
				...
				CScriptArgument result = g_pEvents->Call("serverQueryReceived", &pArguments);

				// Was the query refused?
				if((result.GetType() == SCRIPT_TYPE_INTEGER && result.GetInteger() == 0) || (result.GetType() == SCRIPT_TYPE_STRING && strlen(result.GetString()) == 0))
				{
				...
				if(result.GetType() == SCRIPT_TYPE_STRING)
				{
